...
class Standard extends \Tegs\Implementation
{
    ...
    protected function _implode($args, $scope)
    {
        $glue = self::_expression(array($args[0]),$scope);
        $array = self::_expression(array($args[1]),$scope);
        if (!\is_array($array) || !\is_string($glue)) {
            throw $this->_getExceptionForValue($args[0]." or ".$args[1], "string and array");
        }
    ...
    protected function _remove($var)
    {
        return \nl2br($var);
    }
